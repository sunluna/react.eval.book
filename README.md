...
A: react.eval 实现组件传值调用，借鉴传统的jQuery组件设计风格，代码量少，并且代码逻辑集中在被封装的组件单文件中，同一界面多处复用组件时，只需要指定不同的 id 即可准确调用指定组件内部的方法。每封装一种组件，冗余代码就会大量减少。不受组件层级制约，任意访问。可以以搭积木的方式封装复合组件。API少，方法名好记。

dva/redux实现组件传值，代码分散在models和routes中。同一界面多处复用组件时，需要dispatch增加参数识别同一组件不同实例，再到model里面修改不同的state\(reducers\)，渲染时也要按不同参数配置到组件上\(mapStateToProps\)，才能准确控制指定的组件。过程简单但繁琐，容易出错。并且每封装一种组件，就需要修改至少两个js的代码。虽然同样不受层级制约，但是实际应用中，一般不会写很多model/reducer/store\(因代码冗长\)，属性和回调层层传递，维护不便。概念众多，不符合记忆习惯。
...
**react.eval 存在的价值是，允许定点精准更新目标组件，以最简单的方式耦合小组件，可以根据业务需要复合出不同规格的高级组件。**
