...
react.eval 不承担统一状态管理需求，设计之初就只负责直观的组件通信，无法状态回溯。

最适合再封装纯技术组件，用来集成常见的标签页、表格\(排序、分页、增删改\)、无限滚动、下拉刷新等固定重复需求，

_组件状态可以分为（业务状态、技术状态）。_

_技术状态是指用来记录 控件显示隐藏、当前激活的标签页、页码、每页条数、列表数据、当前选中项等等无聊的状态值，_

_业务状态是指用户信息、交易数据、流程数据等这些真正值钱的状态。_

通过react.eval封装纯技术组件，将技术状态剥离到技术组件内部维护，可以简化状态维护工作，更加专注于真正的业务逻辑。

如果项目中已经整体应用dva/redux ，那么react.eval只推荐封装技术组件，仅用于组件内部子组件状态协调。

不建议在组件外部混用dva/redux 和react.eval，功能都能实现，混用会导致代码可读性差。

如果项目中没有整体应用dva/redux ，那么react.eval 可以承担所有的组件通信。
