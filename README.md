...
dva/redux 没有递进的状态，因为都是从属性里来的，更新就是重新接收属性。

react.eval 调用有状态组件自身方法更新状态，状态是递进变化的。

dva/redux 的组件是属性驱动，一般是无状态组件，组件只负责展示，状态逻辑分离。

react.eval 不做统一的状态管理，状态不分离，只负责组件通信，不做状态回溯。
